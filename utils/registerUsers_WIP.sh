...
CODE=$(cat ../survivers/withSearchingForGroups.lua)
...
registerUser() {
    ...
    curl -s -X POST -b cookie.txt --header "X-CSRFToken: $csrfMiddlewareToken" --header "X-Requested-With: XMLHttpRequest" -F "action=run" -F "code=$CODE" -F "comment=null" -F "parent=null" "localhost/snake/edit/latest" > /dev/null # --trace-ascii /dev/stdout # > /dev/null
...
registerAllUsers() {
    cat ../usernames.txt | while read username
    do
